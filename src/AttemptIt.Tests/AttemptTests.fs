...
let ``An empty attempt should be successful`` () =
    attempt {return ()} |> Attempt.Run |> isSuccess |> should be True

[<Fact>]
let ``An attempt with an exception and no catch should still throw`` () =
    (fun() -> attempt { raise(Exception("Should not be caught")); return ()} |>  Attempt.Run |> ignore) |> should throw typeof<Exception>

[<Fact>]
let ``An attempt with an exception and a catch shoudld return Fail`` () =
    let exceptionHandler (e:exn) = Fail e.Message
    attempt { raise(Exception("Should be caught")); return ()} |> Attempt.Catch exceptionHandler |> Attempt.Run |> isFailure |> should be True
...
let ``A series of success attempts should result in an overall success`` () =
    ...
    let overallAttempt =
        attempt {
            let! a = successfullFunc 1
            let! b = successfullFunc a
            let! c = successfullFunc b
            return! successfullFunc c
        }
    overallAttempt |> Attempt.Run |> successValue |> should equal 5
...
let ``A series of success attempts that fail should result in an overall fail`` () =
    ...
    overallAttempt |> Attempt.Run |> failValue |> should equal 3